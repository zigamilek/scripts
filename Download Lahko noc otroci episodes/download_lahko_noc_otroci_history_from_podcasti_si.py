...
def get_all_episode_links(base_url, driver, existing_links):
    ...
    all_links = []
    page_number = 12  # Start at page 12
    while page_number <= 53:  # End at page 53
...
def extract_episode_links(soup):
    ...
    episode_links = []
    items = soup.find_all('div', class_='episode')
    for item in items:
        link_tag = item.find('a', href=True, class_='text-base')
        if link_tag:
            link = link_tag['href']
            if link:
                episode_links.append("https://podcasti.si" + link)
...
def main(output_folder):
    base_url = 'https://podcasti.si/lahko-noc-otroci/'
    links_file_path = os.path.join(output_folder, "0_all_episode_links.txt")
