...
# Removed Selenium-based fetch; use requests
# Removed Selenium-based parse; use BeautifulSoup
#endregion
...
def download_mp3(mp3_url, date, title, author, narrator, year_of_recording, description, output_folder, downloaded_files, episode_link, counter):
    ...
    response = session.get(mp3_url, headers=headers, stream=True)
...
def build_podcasti_listing(limit=None):
    ...
    logging.info(f"Updated podcasti.si episodes JSON with {len(episodes)} new entries; total {len(episodes)}")
    return episodes

def enrich_podcasti_listing(episodes, limit=None):
    ...
    logging.info(f"Enriched podcasti.si JSON with mp3 URLs; total {len(enriched)} entries (added {count})")
    return enriched
...
def build_rtvslo_listing(limit=None):
...
def enrich_rtvslo_listing(episodes, downloaded_set, force_enrich, limit=None):
...
def main(output_folder, dry_run):
    ...
    podcasti_eps = enrich_podcasti_listing(podcasti_eps, args.limit)
